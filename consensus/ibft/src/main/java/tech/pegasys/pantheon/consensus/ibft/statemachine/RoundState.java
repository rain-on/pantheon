  ...
  public boolean setProposedBlock(final Proposal msg) {
    ...
    if (!proposalMessage.isPresent()) {
      if (validator.addSignedProposalPayload(msg.getSignedPayload())) {
        ...
        prepareMessages.removeIf(p -> !validator.validatePrepareMessage(p.getSignedPayload()));
        commitMessages.removeIf(p -> !validator.validateCommmitMessage(p.getSignedPayload()));
        updateState();
        return true;
      }
    }
